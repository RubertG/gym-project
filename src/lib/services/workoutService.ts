...
export async function getSessionsByUser(userId: string): Promise<WorkoutSession[]> {
...
export async function createSession(
    ...
    if (!isValidDate(sessionDate)) {
        throw new ValidationError('La fecha de sesión no es válida (YYYY-MM-DD)');
    }
...
export async function addSetsToSession(
    ...
    if (session.status !== 'draft') {
        throw new ValidationError('Solo se pueden agregar series a sesiones en estado borrador');
    }
    ...
    for (const s of sets) {
        if (s.reps <= 0) {
            throw new ValidationError('Las repeticiones deben ser mayores a cero');
        }
        if (s.weightKg < 0) {
    ...
    return workoutRepo.createWorkoutSets(db, { workoutSessionId: sessionId, exerciseId, sets });
}

/* ─────────── completar sesión ─────────── */

export async function completeSession(sessionId: string): Promise<WorkoutSession> {
    const db = getServerClient();
...
export async function getCalendarData(
    ...
    const sessions = await workoutRepo.findSessionsInDateRange(db, userId, start, end);
...
export async function calculatePRs(
    ...
    const db = getServerClient();
    const sets = await workoutRepo.findSetsByUserAndExercise(db, userId, exerciseId);
    ...
    let maxWeight = 0;
    let maxVolume = 0;
    let bestSet: { reps: number; weightKg: number; volume: number } | null = null;
